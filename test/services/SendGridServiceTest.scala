...
class SendGridServiceTest extends PlaySpecification with Mockito {
  ...
  "SendGrid Service" should {
    ...
    "send email when receipt is approved" in {
      ...
      val result = sendGridObj.sendEmailForApproval("test@example.com", "test", "rewardLink")
    ...
    "send email for user support" in {
      ...
      val result = sendGridObj.sendEmailForSupport("test", "Support@receiptprocessor.com", "message")
